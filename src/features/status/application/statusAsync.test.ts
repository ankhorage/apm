  ...
  roots: [
    {
      ...
      rootPath: '/fixture',
      packagePaths: ['.'],
      manager: { state: 'selected', name: 'npm', version: '12.0.2', source: 'package-manager-field' },
      lockfile: { state: 'supported', path: 'package-lock.json', format: 'npm-package-lock', version: '3', evidence: ['package-lock.json'] },
      declarations: [
        { ownerPath: 'package.json', name: 'direct', range: '^2.0.0', kind: 'dependency', resolvedPackageId: 'node_modules/direct' },
      ],
      lockedPackages: [
        {
          ...
          source: 'registry',
          optional: false,
          dependencies: [{ name: 'transitive', requested: '^1.0.0', packageId: 'node_modules/transitive' }],
        },
        {
      ...
      installedPackages: [
        { packageId: 'node_modules/direct', state: 'present', source: 'node-modules', version: '1.0.0' },
        { packageId: 'node_modules/transitive', state: 'present', source: 'node-modules', version: '1.0.0' },
        { packageId: 'node_modules/missing', state: 'absent', source: 'node-modules', reason: 'package directory missing' },
      ],
      complete: true,
      diagnostics: [],
    },
...
const availability: ApmAvailabilityEvidence = {
  ...
  diagnostics: [],
  packages: [
    { packageId: '.::node_modules/direct', name: 'direct', state: 'known', latestVersion: '3.0.0', compatibleVersion: '2.5.0' },
    { packageId: '.::node_modules/transitive', name: 'transitive', state: 'known', latestVersion: '1.1.0', compatibleVersion: '1.1.0' },
    { packageId: '.::node_modules/missing', name: 'missing', state: 'known', latestVersion: '1.0.0', compatibleVersion: '1.0.0' },
  ],
};
...
const extensions: ApmExtensionEvidence = {
  ...
  complete: true,
  observations: [
    { packageId: '.::node_modules/direct', projection: 'stale', migration: 'pending', evidence: ['generated projection differs'] },
  ],
  diagnostics: [],
};
...
describe('statusAsync', () => {
  test('distinguishes declaration, lock, installation, transitive, projection, and migration drift', async () => {
    const result = await statusAsync({ rootPath: '/fixture' }, createPorts({ inventory, availability, extensions }));
    ...
    expect(result.dependencies.find((dependency) => dependency.name === 'transitive')?.dependencyPaths).toEqual([
  ...
  test('never reports current when registry availability is unknown', async () => {
    ...
    const result = await statusAsync(
      ...
      createPorts({ inventory, availability: unknownAvailability, extensions: { ...extensions, observations: [] } }),
    );
  ...
  test('never reports current when Project Detector inspection is truncated', async () => {
    const result = await statusAsync(
      { rootPath: '/fixture' },
      createPorts({
        inspection: { ...inspection, complete: false, diagnostics: [{ code: 'entry-budget', message: 'truncated' }] },
        inventory,
        availability,
  ...
  test('keeps host updates separate from application dependency updates', async () => {
    const hostAvailability: ApmAvailabilityEvidence = {
      ...
      packages: [
        ...
        { packageId: 'host:apm', name: '@ankhorage/apm', state: 'known', latestVersion: '0.2.0', compatibleVersion: '0.2.0' },
      ],
    };
    const result = await statusAsync(
      { rootPath: '/fixture', hostPackages: [{ id: 'apm', name: '@ankhorage/apm', version: '0.1.0' }] },
      createPorts({ inventory, availability: hostAvailability, extensions: { ...extensions, observations: [] } }),
    );
