...
describe('inspectDependencyInventoryAsync', () => {
    ...
    await withFixture(async (root) => {
      ...
      await writeFile(
        ...
        JSON.stringify({
          lockfileVersion: 2,
          configVersion: 1,
          workspaces: { '': { name: 'fixture', dependencies: { dep: '^1.0.0' } } },
  ...
  test('keeps nested apps with their own package manager as independent install roots', async () => {
    await withFixture(async (root) => {
      ...
      await writeFile(
        path.join(root, 'packages/app/bun.lock'),
        JSON.stringify({
          lockfileVersion: 2,
          configVersion: 1,
          workspaces: { '': { name: 'nested', dependencies: { dep: '^1.0.0' } } },
          packages: { dep: ['dep@1.0.0', '', {}, 'sha512-fixture'] },
        }),
      );
